...
export default function CustomButton({ children, type, disabled, ...buttonProps }: ButtonProps) {
    return (
        <button
            ...
            type={type ?? 'button'}
            disabled={disabled}
            className={`flex h-full w-full items-center justify-center gap-3 rounded-md border border-slate-600 bg-slate-600 px-5 py-2 text-sm text-white transition-opacity hover:opacity-75 ${
